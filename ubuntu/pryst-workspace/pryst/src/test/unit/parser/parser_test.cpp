...
public:
    ...
    virtual std::any visitMemberAccessExpr(PrystParser::MemberAccessExprContext *ctx) override {
        std::cout << "Visiting member access expression" << std::endl;
        return visitChildren(ctx);
    }
...
TEST_F(ParserTest, ParseTestFile) {
    auto tree = parseFile("unit/parser/parser_test.pst");
...
TEST_F(ParserTest, ParseFunctionDeclaration) {
    auto tree = parseFile("unit/parser/function_test.pst");
...
TEST_F(ParserTest, ParseClassDeclaration) {
    auto tree = parseFile("unit/parser/class_test.pst");
